...
function PlayerRow({ p, onPick }) {
  return (
    <div style={{ display: "flex", gap: 8, alignItems: "center", padding: "6px 0", borderBottom: "1px solid #222" }}>
      ...
      <div style={{ flex: 1 }} />
      {onPick && <button onClick={() => onPick(p)} title="Pick this player">Pick</button>}
    </div>
  );
}
...
export default function App() {
  ...
  const [makePickForm, setMakePickForm] = useState({
    round_no: 1, overall_no: 1, team_slot_id: 1, player_id: ""
  });
  ...
  const nextOverall = useMemo(() => (picks.length ? Math.max(...picks.map(p => p.overall_no)) + 1 : 1), [picks]);
  ...
  const onPick = async (player) => {
    // Basic: fill the form for quick pick, you can also post directly
    const payload = {
  ...
  return (
    <div style={{ padding: 16, fontFamily: "system-ui, Arial", color: "#ddd", background: "#0e0e10", minHeight: "100vh" }}>
      <h2>Draft Room (Minimal)</h2>
      <div style={{ marginBottom: 12, opacity: 0.8 }}>
        API: <code>{import.meta.env.VITE_API_URL}</code> &nbsp;|&nbsp; Health: {health ? "OK" : "…"}
        &nbsp;|&nbsp;
        <button onClick={reloadAll} disabled={loading}>{loading ? "Loading…" : "Refresh"}</button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <Section title="Setup">
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            ...
            <button onClick={async () => { await api.importDemo(); await reloadAll(); }}>Seed Demo Players</button>
            <span style={{ opacity: 0.75 }}>Teams: {teams.length} | Picks: {picks.length}</span>
          </div>
        ...
        <Section title="Make Pick (manual form)">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
            <div>
              <div>Round</div>
              <input type="number" value={makePickForm.round_no}
                     onChange={(e) => setMakePickForm(f => ({ ...f, round_no: Number(e.target.value) }))}/>
            </div>
            <div>
              <div>Overall</div>
              <input type="number" value={makePickForm.overall_no || nextOverall}
                     onChange={(e) => setMakePickForm(f => ({ ...f, overall_no: Number(e.target.value) }))}/>
            </div>
            <div>
              <div>Team Slot</div>
              <input type="number" value={makePickForm.team_slot_id}
                     onChange={(e) => setMakePickForm(f => ({ ...f, team_slot_id: Number(e.target.value) }))}/>
            </div>
            <div>
              <div>Player ID</div>
              <input value={makePickForm.player_id}
                     onChange={(e) => setMakePickForm(f => ({ ...f, player_id: e.target.value }))}/>
            </div>
          </div>
          <div style={{ marginTop: 8 }}>
        ...
        <Section title="Suggestions — Top 3">
          {suggestTop.length === 0 ? <div>No suggestions</div> :
            suggestTop.map(p => <PlayerRow key={p.player_id} p={p} onPick={onPick} />)}
        </Section>

        <Section title="Next 10">
          {suggestNext.length === 0 ? <div>—</div> :
            suggestNext.map(p => <PlayerRow key={p.player_id} p={p} onPick={onPick} />)}
        </Section>

        <Section title="Picks">
          {picks.length === 0 ? <div>No picks yet</div> :
            picks.map(pk => (
              <div key={pk.pick_id} style={{ display: "flex", gap: 8, alignItems: "center", padding: "6px 0", borderBottom: "1px solid #222" }}>
